...
### シミュレーション結果
* プロセスノイズQと観測ノイズRの数値を変えて実験を行った．
* Q = 1.0, R = 1.0の結果を下に示す．
![result1](image/result.png)
* このグラフはそれぞれ以下のことを表している．
	* True Position(青い線): ノイズを含むロボットの位置
	* Observation(赤い点): 観測ノイズを含んだ観測値
	* Kalman Estimate(緑の破線): カルマンフィルタによる推定位置
* Q = 1.0, R = 1.0の結果を下に示す．
![result2](image/result.png)
* ノイズの大きい観測値よりもカルマンフィルタがロボット位置を追従していることがわかる．
* 観測値はノイズの影響を受けており、そのままでは信頼できないが、
* カルマンフィルタによりノイズが低減され、
* 真の状態に近い推定が得られている。
* カルマンフィルタが「ノイズ除去器」として機能していることが確認できる
## 使用ライブラリ
* Python3
	* テスト済み: 3.9~3.11
* Numpy
