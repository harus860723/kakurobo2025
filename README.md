...
\end{align}
```
* 観測値zkは，位置xkを直接観測せずにセンサノイズを含む．
#### カルマンフィルタ
* 状態推定には1次元カルマンフィルタを用いる．
* カルマンフィルタは推定結果，観測値，分散を使い最も妥当な推定値を計算する．
* 予測ステップ
	* 推定値: ^xk
	* 推定誤差分散: Pk
	* 時刻k-1までの情報を使って予測した時刻kの位置: ^xk|k-1
...
```math
\begin{align}
\hat{x} = \hat{x}_{k|k-1} + K_k(z_k - \hat{x}_{x|k-1})
\end{align}
```
```math
\begin{align}
...
### シミュレーション結果
* 観測ノイズRがカルマンフィルタの推定結果に与える影響を調査するため，数値を変えて実験を行った．
* R = 1.0の結果を下に示す．
![result1](image/result1.png)
* このグラフはそれぞれ以下のことを表している．
	* True Position(青い線): ノイズを含むロボットの位置
	* Observation(赤い点): 観測ノイズを含んだ観測値
	* Kalman Estimate(緑の破線): カルマンフィルタによる推定位置
* 次に，観測ノイズを大きくして実験を行った．
* R = 20.0の結果を下に示す．
![result2](image/result2.png)
* ノイズの大きい観測値よりもカルマンフィルタが，ロボット位置を追従していることがわかる．
* 観測値はノイズの影響を受けておりそのままでは信頼できないが，カルマンフィルタによりノイズが低減されロボットの状態に近い推定が得られている．
* カルマンフィルタは，観測とモデルのどちらを重視するかを変化させることが確認できた．
